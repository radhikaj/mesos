...
create_attributes_list_for_handles(const std::vector<HANDLE>& handles)
{
  ...
  BOOL result = ::InitializeProcThreadAttributeList(
    nullptr, // Pointer to _PROC_THREAD_ATTRIBUTE_LIST. This parameter can be
             // NULL to determine the buffer size required to support the
  ...
  result = ::UpdateProcThreadAttribute(
    attribute_list.get(), // Pointer to _PROC_THREAD_ATTRIBUTE_LIST.
